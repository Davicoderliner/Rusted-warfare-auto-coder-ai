...
import { generateUnit, editCodeWithGemini } from './services/geminiService';
import type { ChatMessage, GeneratedUnit, Mod } from './types';
import { Bot, Code2, Image as ImageIcon, Wind, Download, Package } from 'lucide-react';
          ...
          <div className="flex items-center gap-4">
             {currentMod && currentMod.units.length > 0 && (
                <div className="flex items-center gap-2 text-sm text-gray-300 bg-gray-800/50 px-3 py-1.5 rounded-md">
                    ...
                    Mod: <span className="font-semibold text-white">{currentMod.name}</span> (<span className="font-semibold text-white">{currentMod.units.length}</span> unit{currentMod.units.length > 1 ? 's' : ''})
                </div>
             )}
            <button
